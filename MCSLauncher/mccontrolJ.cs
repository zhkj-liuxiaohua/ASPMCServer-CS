...
// 此文件用于变通Java版(官方服务端)的使用
// 需要更改配置文件 web.config 启动参数，java ，java路径，java参数，地图文件名等。不适用于多个java服。
// 若非必要，请自行调整参数
...
//		public const string INPUTFLAG_TAG = "MINPUTFLAG_TAG";
		...
		private static void OnDataReceived(object sender, DataReceivedEventArgs e)
		{
			...
			// 适配Java服
			if(chatservice != null) {
				int chatindex = info.IndexOf("INFO]: <");
					if (chatindex > -1) {	// 聊天消息，组合成bds的chat消息再转发
					string subchat = info.Substring(chatindex + 8);
					int vict = subchat.IndexOf('>');
					string pname = null;
					string msg = null;
					if (vict > 0) {
						string stime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
						pname = subchat.Substring(0, vict);
						msg = subchat.Substring(vict + 2);
						string ev = string.Format("[{0} Chat] 玩家 {1} 说:{2}", stime, pname, msg);
						chatSendFarMsg(ev);
						eventAdd(ev);
					}
				}
		...
		private static void checkChatPlayerPlays(string info) {
			...
			int ci = info.IndexOf("joined the game");
			if (ci > -1) {
				// 解析连接数据
				string x = info.Substring(info.IndexOf("INFO]: ") + 7);
				string name = x.Substring(0, x.IndexOf("joined the game") - 1);
				//string[] pinfo = info.Substring(ci).Split(',');
				//if (pinfo.Length == 2) {
				//	string[] pids = pinfo[0].Split(':');
				//	string name = "";
			...
			ci = info.IndexOf("left the game");
			if (ci > -1) {
				// 解析断开数据
				string x = info.Substring(info.IndexOf("INFO]: ") + 7);
				string name = x.Substring(0, x.IndexOf("left the game") - 1);
				//string[] pinfo = info.Substring(ci).Split(',');
				//if (pinfo.Length == 2) {
				//	string[] pids = pinfo[0].Split(':');
				//	string name = "";
		...
		private static void startProcThread()
		{
			while (keeprun) {
				...
				myProcess.StartInfo.StandardOutputEncoding = Encoding.Default;
				myProcess.OutputDataReceived += OnDataReceived;
				myProcess.StartInfo.WorkingDirectory = Path.GetDirectoryName(Process.GetCurrentProcess().MainModule.FileName);//Path.GetDirectoryName(procpath);//
				myProcess.Start();
